...
class SecureMessagingTest {
    ...
    private val secureMessaging = SecureMessaging(paceKey, ByteArray(BLOCK_SIZE))
    ...
    fun shouldDecryptDo87Apdu() {
        // Initialize secure messaging with the paceKey
        val secureMessaging = SecureMessaging(paceKey, ByteArray(BLOCK_SIZE))

        // APDU to decrypt (encrypted response)
        val apduToDecrypt = ResponseApdu(Hex.decode("871101496c26d36306679609665a385c54db37990290008E08B7E9ED2A0C89FB3A9000"))
        ...
        println("Decrypted APDU: ${decryptedApdu.bytesInHex()}")
        println("Expected APDU: ${expectedDecryptedApdu.bytesInHex()}")

        // Assert that the decrypted APDU matches the expectation
        Assert.assertArrayEquals(
            "The decrypted APDU does not match the expected value.",
            expectedDecryptedApdu.bytes,
            decryptedApdu.bytes
        )
    }
