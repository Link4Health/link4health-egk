...
fun Project.registerDocumentationTasks() {
    tasks.register("generateDocIndexHtml") {
        ...
        doLast {
            try {
                ...
                val readmeHtml = runCommandWithInput(
                    input = readmeContent,
                    PANDOC,
                    "-f",
                    "markdown",
                ...
                val documentationVersionsDir = File(outputDirectory, "releases")
                    .listFiles { candidate ->
                        candidate.isDirectory && candidate.name.matches(Regex("v\\d+\\.\\d+\\.\\d+"))
                    }
                    ?.map { it.name }
                    ?.toMutableSet()
                    ?: mutableSetOf()
                ...
                val sortedVersions = documentationVersionsDir
                ...
                releaseNotesFullPath.parentFile.mkdirs()
...
private class VersionComparator : Comparator<String> {
    override fun compare(firstVersion: String, secondVersion: String): Int {
        val version1 = firstVersion.removePrefix("v").split(".").map { it.toInt() }
        val version2 = secondVersion.removePrefix("v").split(".").map { it.toInt() }

        for (index in 0 until maxOf(version1.size, version2.size)) {
            val part1 = version1.getOrNull(index) ?: 0
            val part2 = version2.getOrNull(index) ?: 0
            if (part1 != part2) {
                return part1 - part2
            }
        }
...
private fun copyFiles(sourceDir: String, destDir: String) {
...
private fun runCommandWithInput(input: String, vararg command: String): String {
...
private fun Project.getVersionFromProperties(): String {
